...
public class Worker : BackgroundService
{
    ...
    public Worker(IOikDataApi api, IHostApplicationLifetime applicationLifetime)
    {
        _api = api;
    ...
    public override async Task StartAsync(CancellationToken cancellationToken)
    {
        try
        {
            ...
            _commandTexts.AddRange(
                _config.SqlText.Split(
                    ';',
    ...
    private async Task DoWork(CancellationToken stoppingToken)
    {
        ...
        try
        {
            ...
            foreach (var rawCommandText in _commandTexts)
            {
                ...
                {
                    var rowsCount = await db.ExecuteAsync(
                        new CommandDefinition(commandText, cancellationToken: stoppingToken)
                    );
